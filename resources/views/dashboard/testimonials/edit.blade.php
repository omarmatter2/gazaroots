...
<div class="container-fixed p-5">
    <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
        <div class="flex flex-col justify-center gap-2">
            <h1 class="text-xl font-semibold leading-none text-mono">Edit Testimonial</h1>
            <div class="flex items-center gap-2 text-sm font-medium text-secondary-foreground">
                ...
                <span>/</span>
                <span>{{ $testimonial->getTranslation('name', 'en') }}</span>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="kt-card">
                ...
                <div class="grid gap-5 lg:gap-7.5">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        <div class="flex flex-col gap-1">
                            ...
                            <input type="text" class="kt-input" id="name_en" name="name[en]" value="{{ old('name.en', $testimonial->getTranslation('name', 'en')) }}" required>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="name_ar">Name (Arabic) <span class="text-danger">*</span></label>
                            <input type="text" class="kt-input" id="name_ar" name="name[ar]" value="{{ old('name.ar', $testimonial->getTranslation('name', 'ar')) }}" dir="rtl" required>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="content_en">Content (English) <span class="text-danger">*</span></label>
                            <textarea class="kt-textarea" id="content_en" name="content[en]" rows="4" required>{{ old('content.en', $testimonial->getTranslation('content', 'en')) }}</textarea>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="content_ar">Content (Arabic) <span class="text-danger">*</span></label>
                            <textarea class="kt-textarea" id="content_ar" name="content[ar]" rows="4" dir="rtl" required>{{ old('content.ar', $testimonial->getTranslation('content', 'ar')) }}</textarea>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <img src="{{ Storage::url($testimonial->image) }}" class="w-16 h-16 rounded-full object-cover border">
                                @endif
                                <label class="flex flex-col items-center justify-center w-16 h-16 border-2 border-dashed border-border rounded-full cursor-pointer hover:bg-muted/50 transition-colors overflow-hidden">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <input type="number" class="kt-input" id="order" name="order" value="{{ old('order', $testimonial->order) }}" min="0">
                        </div>
                        <div class="flex flex-col gap-2">
                            <label class="kt-form-label">Status</label>
                            <div class="flex items-center gap-2.5">
                                <input type="hidden" name="is_active" value="0">
                                <input type="checkbox" class="kt-switch" name="is_active" value="1" {{ old('is_active', $testimonial->is_active) ? 'checked' : '' }}>
            ...
            <div class="kt-card-footer flex justify-end gap-2.5 p-7.5 border-t border-border">
                ...
                <button type="submit" class="kt-btn kt-btn-primary">Update Testimonial</button>
            </div>
        </div>
    </form>
</div>
