...
<div class="container-fixed p-5">
    <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
        <div class="flex flex-col justify-center gap-2">
            <h1 class="text-xl font-semibold leading-none text-mono">Edit Author</h1>
            <div class="flex items-center gap-2 text-sm font-medium text-secondary-foreground">
                ...
                <span>/</span>
                <span>{{ $author->getTranslation('name', 'en') }}</span>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.authors.update', $author) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="kt-card">
                ...
                <div class="grid gap-5 lg:gap-7.5">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        <div class="flex flex-col gap-1">
                            ...
                            <input type="text" class="kt-input" id="name_en" name="name[en]" value="{{ old('name.en', $author->getTranslation('name', 'en')) }}" required>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="name_ar">Name (Arabic) <span class="text-danger">*</span></label>
                            <input type="text" class="kt-input" id="name_ar" name="name[ar]" value="{{ old('name.ar', $author->getTranslation('name', 'ar')) }}" dir="rtl" required>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="email">Email</label>
                            <input type="email" class="kt-input" id="email" name="email" value="{{ old('email', $author->email) }}">
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="phone">Phone</label>
                            <input type="text" class="kt-input" id="phone" name="phone" value="{{ old('phone', $author->phone) }}">
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="location">Location</label>
                            <input type="text" class="kt-input" id="location" name="location" value="{{ old('location', $author->location) }}">
                        </div>
                    </div>
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="bio_en">Bio (English)</label>
                            <textarea class="kt-textarea" id="bio_en" name="bio[en]" rows="4">{{ old('bio.en', $author->getTranslation('bio', 'en')) }}</textarea>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="bio_ar">Bio (Arabic)</label>
                            <textarea class="kt-textarea" id="bio_ar" name="bio[ar]" rows="4" dir="rtl">{{ old('bio.ar', $author->getTranslation('bio', 'ar')) }}</textarea>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="flex items-start gap-4">
                                @if($author->avatar)
                                <img src="{{ Storage::url($author->avatar) }}" class="w-20 h-20 rounded-full object-cover border">
                                @endif
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="flex items-center gap-2.5">
                                ...
                                <input type="checkbox" class="kt-switch" name="is_active" value="1" {{ old('is_active', $author->is_active) ? 'checked' : '' }}>
            ...
            <div class="kt-card-footer flex justify-end gap-2.5 p-7.5 border-t border-border">
                ...
                <button type="submit" class="kt-btn kt-btn-primary">Update Author</button>
            </div>
        </div>
    </form>
</div>
