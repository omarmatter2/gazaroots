...
<div class="container-fixed p-5">
    <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
        <div class="flex flex-col justify-center gap-2">
            <h1 class="text-xl font-semibold leading-none text-mono">Create Article</h1>
                    ...
                    <div class="kt-card-content grid gap-5">
                        <div class="grid grid-cols-2 gap-5">
                            <div class="flex flex-col gap-1">
                                ...
                                <input type="text" class="kt-input" id="title_en" name="title[en]" value="{{ old('title.en') }}" required>
                            </div>
                            <div class="flex flex-col gap-1">
                                <label class="kt-form-label" for="title_ar">Title (Arabic) <span class="text-danger">*</span></label>
                                <input type="text" class="kt-input" id="title_ar" name="title[ar]" value="{{ old('title.ar') }}" dir="rtl" required>
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-5">
                            <div class="flex flex-col gap-1">
                                <label class="kt-form-label" for="excerpt_en">Excerpt (English)</label>
                                <textarea class="kt-textarea" id="excerpt_en" name="excerpt[en]" rows="2">{{ old('excerpt.en') }}</textarea>
                            </div>
                            <div class="flex flex-col gap-1">
                                <label class="kt-form-label" for="excerpt_ar">Excerpt (Arabic)</label>
                                <textarea class="kt-textarea" id="excerpt_ar" name="excerpt[ar]" rows="2" dir="rtl">{{ old('excerpt.ar') }}</textarea>
                            </div>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="content_en">Content (English) <span class="text-danger">*</span></label>
                            <textarea class="tinymce-editor" id="content_en" name="content[en]">{{ old('content.en') }}</textarea>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="content_ar">Content (Arabic) <span class="text-danger">*</span></label>
                            <textarea class="tinymce-editor-rtl" id="content_ar" name="content[ar]">{{ old('content.ar') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <select class="kt-select" id="category_id" name="category_id" required>
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }}>{{ $category->getTranslation('name', 'en') }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="flex flex-col gap-1">
                            ...
                            <select class="kt-select" id="author_id" name="author_id" required>
                                ...
                                @foreach($authors as $author)
                                    <option value="{{ $author->id }}" {{ old('author_id') == $author->id ? 'selected' : '' }}>{{ $author->getTranslation('name', 'en') }}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="kt-card">
                    ...
                    <div class="kt-card-content grid gap-4">
                        <div class="flex items-center justify-between">
                            ...
                            <input type="checkbox" class="kt-switch" name="is_published" value="1" {{ old('is_published') ? 'checked' : '' }}>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm">Featured</span>
                            <input type="hidden" name="is_featured" value="0">
                            <input type="checkbox" class="kt-switch" name="is_featured" value="1" {{ old('is_featured') ? 'checked' : '' }}>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm">Urgent News</span>
                            <input type="hidden" name="is_urgent" value="0">
                            <input type="checkbox" class="kt-switch" name="is_urgent" value="1" {{ old('is_urgent') ? 'checked' : '' }}>
                        </div>
                    </div>
                </div>
                <div class="flex gap-2.5">
                    <a href="{{ route('admin.articles.index') }}" class="kt-btn kt-btn-outline flex-1">Cancel</a>
                    <button type="submit" class="kt-btn kt-btn-primary flex-1">Create</button>
                </div>
            </div>
        </div>
    </form>
