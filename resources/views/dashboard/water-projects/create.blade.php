...
@section('title', 'Create Water Project')

@section('content')
<div class="container-fixed p-5">
    <div class="flex flex-wrap items-center lg:items-end justify-between gap-5 pb-7.5">
        <div class="flex flex-col justify-center gap-2">
            <h1 class="text-xl font-semibold leading-none text-mono">Create Water Project</h1>
    ...
    <form action="{{ route('admin.water-projects.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="kt-card">
                ...
                <div class="grid gap-5 lg:gap-7.5">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        <div class="flex flex-col gap-1">
                            ...
                            <textarea class="kt-textarea" id="description_en" name="description[en]" rows="4">{{ old('description.en') }}</textarea>
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="description_ar">Description (Arabic)</label>
                            <textarea class="kt-textarea" id="description_ar" name="description[ar]" rows="4" dir="rtl">{{ old('description.ar') }}</textarea>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-5">
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="wells_built">Wells Built</label>
                            <input type="number" class="kt-input" id="wells_built" name="wells_built" value="{{ old('wells_built', 0) }}" min="0">
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="beneficiaries">Beneficiaries</label>
                            <input type="number" class="kt-input" id="beneficiaries" name="beneficiaries" value="{{ old('beneficiaries', 0) }}" min="0">
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="families_served">Families Served</label>
                            <input type="number" class="kt-input" id="families_served" name="families_served" value="{{ old('families_served', 0) }}" min="0">
                        </div>
                        <div class="flex flex-col gap-1">
                            <label class="kt-form-label" for="neighborhoods">Neighborhoods</label>
                            <input type="number" class="kt-input" id="neighborhoods" name="neighborhoods" value="{{ old('neighborhoods', 0) }}" min="0">
                        </div>
                    </div>
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div class="kt-card-footer flex justify-end gap-2.5 p-7.5 border-t border-border">
                ...
                <button type="submit" class="kt-btn kt-btn-primary">Create Project</button>
            </div>
        </div>
    </form>
</div>
